  ...
  function validateForm() {
      ...
      // Check if final outcome option is selected
      var outcomeOptions = document.getElementsByName("outcome");
      var outcomeOptionSelected = false;
      for (var i = 0; i < outcomeOptions.length; i++) {
          if (outcomeOptions[i].checked) {
              outcomeOptionSelected = true;
              break;
          }
      }
      if (!outcomeOptionSelected) {
          errorMessage += "- Final outcome\n";
          isValid = false;
      }

      // Check if facility name is filled when referred to another facility is selected
      var facilityName = document.getElementById("facility").value;
      var facilityCheckbox = document.getElementsByName("outcome")[4];
      if (facilityCheckbox.checked && facilityName === "") {
          errorMessage += "- Facility name\n";
          isValid = false;
      }

      // Check if final outcome date is filled
      var finalOutcomeDate = document.getElementById("final_outcome").value;
      if (finalOutcomeDate === "") {
          errorMessage += "- Date of final outcome\n";
          isValid = false;
      }
